...
console.log('대문자로 변환된 항목들:', uppercaseItems);

// 'a'로 시작하는 항목만 필터링하기
const aItems = items.filter(item => item.startsWith('a'));
console.log("'a'로 시작하는 항목들:", aItems);

// 항목들의 길이 합 계산하기
const totalLength = items.reduce((acc, item) => acc + item.length, 0);
console.log('항목들의 총 길이:', totalLength);`,
  },
  typescript: {
    id: "typescript",
    label: "TypeScript",
